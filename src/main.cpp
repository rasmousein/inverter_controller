...
// Takes analog input (0-4095) and returns voltage calculated from calibration table
float adc_to_v(uint16_t x)
{
  // 10.07kh and 2.169kh resistors for ESP32-TTGO
...
void loop()
{
  ...
  if (millis() - timestamp1 > 100)
  {
    ...
    if (OnOff >= 200.0)
    {
      tft.drawString("Inverter Off", 20, 90, 4);
    }
    else
    {
      tft.drawString("Inverter On", 20, 90, 4);
    }
    timestamp1 = millis();
  }
...
void BTWrite()
{
  SerialBT.print(Vin, 2);
  SerialBT.println("v");
  SerialBT.print("ADC Value Vin   : ");
  SerialBT.println(analogRead(VinPin));
  SerialBT.print("ADC Value OnOff : ");
  SerialBT.println(analogRead(OnOffPin));
  SerialBT.print("CurrentTime         : ");
  SerialBT.println(CurrentTime);
  SerialBT.print("StartSwitchTime     : ");
  SerialBT.println(SwitchStartTime);
  test = CurrentTime - SwitchStartTime;
  SerialBT.print("TimeSinceSwitchTime : ");
  SerialBT.println(test);
  SerialBT.print("SwitchPin  : ");
  SerialBT.println(digitalRead(SwitchPin));
  SerialBT.print("Switching  : ");
  SerialBT.println(Switching);
  SerialBT.print("Sensing    : ");
  SerialBT.println(Sensing);
...
void InverterControl()
{
  ...
  Serial.print(Vin, 2);
  Serial.println("v");
  Serial.print("ADC Value Vin   : ");
  Serial.println(analogRead(VinPin));
  Serial.print("ADC Value OnOff : ");
  Serial.println(analogRead(OnOffPin));
  Serial.print("CurrentTime         : ");
  Serial.println(CurrentTime);
  Serial.print("StartSwitchTime     : ");
  Serial.println(SwitchStartTime);
  test = CurrentTime - SwitchStartTime;
  Serial.print("TimeSinceSwitchTime : ");
  Serial.println(test);
